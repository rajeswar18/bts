...
class DataLoadPreprocess(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            if self.args.smol:
                image = rescale(image, 0.3, anti_aliasing=False).astype(np.float32)
                depth_gt = rescale(depth_gt, 0.3, anti_aliasing=False).astype(np.float32)

            print(image.shape, image.dtype, image.min(), image.max())
            print(depth_gt.shape, depth_gt.dtype, depth_gt.min(), depth_gt.max())

            image, depth_gt = self.random_crop(image, depth_gt, self.args.input_height, self.args.input_width)
            image, depth_gt = self.train_preprocess(image, depth_gt)

            print (image.shape, image.dtype, image.min(), image.max(), image.mean())
            print (depth_gt.shape, depth_gt.dtype, depth_gt.min(), depth_gt.max(), depth_gt.mean())
            print (focal)
...
if __name__ == '__main__':
    ...
    class args:
        ...
        input_width = 128
        input_height = 128
        data_path = "/Volumes/dell/nyu-depth-v2/sync/"
        gt_path = "/Volumes/dell/nyu-depth-v2/sync/"
        filenames_file = "../train_test_inputs/nyudepthv2_train_files_with_gt.txt"

    ds = DataLoadPreprocess(args, "train")
    print(len(ds))
    a = ds[0]